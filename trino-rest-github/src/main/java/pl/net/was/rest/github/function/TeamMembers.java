...
@ScalarFunction(value = "team_members", deterministic = false)
        ...
        extends BaseFunction
{
    ...
    public Block getPage(@SqlType(VARCHAR) Slice org, @SqlType(VARCHAR) Slice teamSlug)
            throws IOException
    {
        ...
        int page = 1;
        while (true) {
            Response<List<Member>> response = service.listOrgTeamMembers(
                    "Bearer " + token,
                    org.toStringUtf8(),
                    teamSlug.toStringUtf8(),
                    PER_PAGE,
                    page++).execute();
            ...
            items.forEach(i -> {
                i.setOrg(org.toStringUtf8());
                i.setTeamSlug(teamSlug.toStringUtf8());
            });
